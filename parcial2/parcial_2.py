...
def fill_adn():
    ...
    for i in range(6):
        ...
        while True:
            ...
            # Se agregara la palabra a la lista "list_letter" y hara un break, sino el programa pedira otra vez que ingrese otra palabra 
    ...
    for rows in range(6):
        ...
        for columns in range(6):
            ...
            if (array[columns][rows] == element):
                ...
                counter += 1
                if (counter >= 4):   # si el contador es mayor o igual a 4 entonces retornara un True y saldra de la funcion.
                    return True
            else:
                # si la secuencia de encontra la linea vertical se rompe se asginara a 0 el contador
                counter = 0
...
def d1(array, element, rows, columns):
    counter = 0  # se inicializara la variable count en 0
    ...
    for i in range(rows, 6):
        for j in range(columns, 6):
            ...
            if (j==i):
                ...
                # Si la condicion se cumple el valor de "counter" aumentara en 1
                if array[i][j] == element:
                    counter += 1
                    if counter >= 4: # si el contador es mayor o igual a 4 entonces retornara el valor de counter y saldra de la funcion.
                        return counter
                else:
                     # si la secuencia de encontra la linea diagonal se rompe se asginara a 0 el contador
                    counter = 0
            else:
                ...
                if product == 1:
                    if array[i][j] == element:
                        ...
                        counter = 0
    # Al final la funcion devolvera el valor de "counter" y se asiganara en la variable "counter" de la funcion diagonally_1
    return counter
...
def d2(array, element, rows, columns, condition):
    counter = 0 # inicializamos la variable counter en 0.
    ...
    for i in range(rows, 6):
        for j in range(0, columns+1):
            ...
            if i+j == condition:
                ...
                # Si cumple entonces se sumara "counter", sino no va a ejecutar nada.
                if (array[i][j] == element):
                    counter += 1
                    if counter >= 4: #Si counter es mayor o igual a 4 retornara el valor de counter y saldra del bucle
                        return counter
                    
                else:
                     # si la secuencia de encontra la linea diagonal se rompe se asginara a 0 el contador
                    counter = 0
    # Al final devolvera a variable counter, que se almacenara en la variable "counter" de la funcion diagonally_2.
    return counter
...
while True:
    ...
    if is_mutant(adn):  
        #Con la funcion is_mutant devolvera un valor True/False, si devuelve un True entonces,
        # se habra encontrado un mutante, y se sumara 1 la variable mutants por cada mutante encontrado.
        print("El ADN es mutante")
        mutants += 1
    else:
        #Si devuelve un False,se mostrara un mensaje que diga que no se encontro un mutante
        print("El ADN no es mutante")
    #Por ultimo tenemos, que informar al usuario si desea encontrar mas mutantes, o si desea
    # Salir del programa
    exit = input("Desea reclutar reclutar mas mutantes <<<s/n>>>: ")
    exit = exit.lower()
    if exit == 's':
        continue
    else:
        break
#una vez se habra haya salido del programa mostrara un mensaje que dira cuantos mutantes reclutamos.
# sino se reclutaron mutantes entonces mostrara un mensaje que lo informe.
if mutants != 0:
    print(f"Felicidades el equipo esta conformado por {mutants} mutantes!!")
else:
    print("NO se reclutaron mutantes!!")
